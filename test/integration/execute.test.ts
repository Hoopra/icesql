import { query, execute, remove, update } from '@src/connection';
...
describe('execute', () => {
  it(`should execute command`, async () => {
    ...
    expect((await query(`SELECT * FROM a`, connector)).length).toBe(0);
  });
...
describe('update', () => {
  it(`should update entry`, async () => {
    ...
    expect((await query<A>(`SELECT * FROM a`, connector))[0]?.text).toBe('b');
  });
});
...
describe('remove', () => {
  ...
  it(`should remove one entry`, async () => {
    ...
    expect((await query(`SELECT * FROM a`, connector)).length).toBe(1);
  });
});
